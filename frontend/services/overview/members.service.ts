import { getClan, getPlayer } from "@/lib/api";
...
export async function getMembersData(tag: string) {
  ...
  const players = await Promise.all(
    clan.memberList.map((member) => getPlayer(member.tag))
  );
...
function calculateHeroProgress(player: Player): number {
  ...
  const currentLevels = homeHeroes.reduce(
    (total, hero) => total + hero.level,
    0
  );

  const maxLevels = homeHeroes.reduce(
    (total, hero) => total + hero.maxLevel,
    0
  );
...
function calculateEquipmentProgress(player: Player): number {
  const homeEquipment = player.heroEquipment.filter(
    (equipment) => equipment.village === "home"
  );

  const currentLevels = homeEquipment.reduce(
    (total, equipment) => total + equipment.level,
    0
  );

  const maxLevels = homeEquipment.reduce(
    (total, equipment) => total + equipment.maxLevel,
    0
  );
...
function calculateTroopProgress(player: Player): number {
  ...
  const currentLevels = homeTroops.reduce(
    (total, troop) => total + troop.level,
    0
  );

  const maxLevels = homeTroops.reduce(
    (total, troop) => total + troop.maxLevel,
    0
  );
...
function calculateSpellProgress(player: Player): number {
  ...
  const currentLevels = homeSpells.reduce(
    (total, spell) => total + spell.level,
    0
  );

  const maxLevels = homeSpells.reduce(
    (total, spell) => total + spell.maxLevel,
    0
  );
